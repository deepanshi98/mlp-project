...
from min2net.preprocessing.MBD2117 import raw
...
def subject_dependent_setting(k_folds, pick_smp_freq, bands, order, save_path, num_class=2, sel_chs=None):
    ...
    n_folds = k_folds
    save_path = save_path + '/MBD2117/time_domain/{}_class/subject_dependent'.format(num_class)
    n_chs = len(sel_chs)
    ...
    for s in range(n_subjs):
        X_train, y_train, X_test, y_test = __load_MBD2117(raw_path, s+1, pick_smp_freq, num_class, id_chosen_chs)
...
def subject_independent_setting(k_folds, pick_smp_freq, bands, order, save_path, num_class=2, sel_chs=None):
    sel_chs = CONSTANT['sel_chs'] if sel_chs == None else sel_chs
    n_folds = k_folds
    save_path = save_path + '/MBD2117/time_domain/{}_class/subject_independent'.format(num_class)
    n_chs = len(sel_chs)
...
def __load_MBD2117(PATH, subject, new_smp_freq, num_class, id_chosen_chs):
