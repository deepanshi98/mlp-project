...
from min2net.preprocessing.MBDMNIST.v17 import raw
from min2net.preprocessing.config import CONSTANT
CONSTANT = CONSTANT['MBDMNIST17']
...
def subject_dependent_setting(k_folds, pick_smp_freq, bands, order, save_path, num_class=2, sel_chs=None):
    ...
    n_folds = k_folds
    save_path = save_path + '/MBDMNIST17/visual/{}_class/subject_dependent'.format(num_class)
    n_chs = len(sel_chs)


    id_chosen_chs = raw.chanel_selection(sel_chs)
    EEG_train, MNIST_train, y_train, EEG_test, MNIST_test, y_test = __load_MBDMNIST17(raw_path, 1, pick_smp_freq, num_class, id_chosen_chs)
