...
def subject_dependent_setting(k_folds, pick_smp_freq, bands, order, save_path, num_class=2, sel_chs=None):
    ...
    n_folds = k_folds
    save_path = save_path + '/MBDMNISTv17/time_domain/{}_class/subject_dependent'.format(num_class)
    n_chs = len(sel_chs)
    ...
    for s in range(n_subjs):
        X_train, y_train, X_test, y_test = __load_MBDMNIST17(raw_path, s+1, pick_smp_freq, num_class, id_chosen_chs)
