...
def subject_dependent_setting(
    k_folds, pick_smp_freq, bands, order, save_path, num_class=2, sel_chs=None
):
    sel_chs = CONSTANT["sel_chs"] if sel_chs == None else sel_chs
    n_folds = k_folds
    save_path = save_path + "/MBDEPV1/time_domain/{}_class/subject_dependent".format(
        num_class
    )
    n_chs = len(sel_chs)
    ...
    for person, (X_tr, y_tr, X_te, y_te) in enumerate(
        ...
        if n_folds == 1:
            X_tr_cv, X_val_cv, y_tr_cv, y_val_cv = train_test_split(
                X_tr, y_tr, test_size=0.2, random_state=42, shuffle=True
            )
            print("Band-pass filtering from {} to {} Hz.".format(bands[0], bands[1]))
            X_tr_fil = butter_bandpass_filter(
                X_tr_cv, bands[0], bands[1], pick_smp_freq, order
            )
            X_val_fil = butter_bandpass_filter(
                X_val_cv, bands[0], bands[1], pick_smp_freq, order
            )
            X_te_fil = butter_bandpass_filter(
                X_te, bands[0], bands[1], pick_smp_freq, order
            )
            print(
                "Check dimension of training data {}, val data {} and testing data {}".format(
                    X_tr_fil.shape, X_val_fil.shape, X_te_fil.shape
                )
            )
            SAVE_NAME = "S{:03d}_fold001".format(person + 1)
            __save_data_with_valset(
                save_path,
                SAVE_NAME,
                X_tr_fil,
                y_tr_cv,
            ...
            for fold, (train_index, val_index) in enumerate(skf.split(X_tr, y_tr)):
                ...
                print(
                    "Band-pass filtering from {} to {} Hz.".format(bands[0], bands[1])
                )
                X_tr_fil = butter_bandpass_filter(
                    X_tr_cv, bands[0], bands[1], pick_smp_freq, order
                )
                ...
                print(
                    "The preprocessing of subject {} from fold {} is DONE!!!".format(
                        person + 1, fold + 1
                    )
                )
...
def __save_data_with_valset(
    save_path, NAME, X_train, y_train, X_val, y_val, X_test, y_test
):
