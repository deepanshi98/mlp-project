...
class BrainVizNet:
    ...
    def build(self):
        ...
        de_conv        = Dense(7*7*32, activation='elu', 
                               kernel_constraint=max_norm(0.5))(decoder_input)
        de_conv        = Reshape((7, 7, 32))(de_conv)
        de_conv        = Conv2DTranspose(filters=64, kernel_size=3, strides=2, padding='same',
                                         activation='relu',
                                         kernel_constraint=max_norm(2., axis=(0, 1, 2)))(de_conv)
        de_conv = Conv2DTranspose(filters=32, kernel_size=3, strides=2, padding='same',
                                         activation='relu',
                                         kernel_constraint=max_norm(2., axis=(0, 1, 2)))(de_conv)
    ...
    def fit(self, EEG_train, MNIST_train, y_train, EEG_val, MNIST_val, y_val):
            
        if EEG_train.ndim != 4:
            raise Exception('ValueError: `EEG_train` is incompatible: expected ndim=4, found ndim='+str(EEG_train.ndim))
        if EEG_val.ndim != 4:
            raise Exception('ValueError: `EEG_val` is incompatible: expected ndim=4, found ndim='+str(EEG_val.ndim))
